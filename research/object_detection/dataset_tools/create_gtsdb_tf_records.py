...
def _find_image_bounding_boxes(directory, cur_record):
  ...
  for line in gt:
      if re.match(cur_record[1][0:5] + "(.*)", line):
          ...
          labels.append(int(dataset_common.GTSDB_LABELS_MAIN[str(int(obj[5])+1)][0])) #let 0 be background class
          labels_text.append(dataset_common.GTSDB_LABELS_MAIN[str(int(obj[5])+1)][1].encode('ascii'))          
          difficult.append(0)
          truncated.append(0)
