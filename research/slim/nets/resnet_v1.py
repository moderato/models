...
def resblock( inputs,
  ...
  Returns:
    The ResNet unit's output.
  """
  with tf.variable_scope(scope, 'resblock_v1', [inputs]) as sc:
    depth_in = slim.utils.last_dimension(inputs.get_shape(), min_rank=4)
    if depth == depth_in:
      shortcut = resnet_utils.subsample(inputs, stride, 'shortcut')
    else:
    ...
    residual = slim.conv2d(inputs, depth, [3, 3], stride=1,
                           scope='conv1')
    residual = slim.conv2d(residual, depth, [3, 3], stride=1,
...
def resnet_v1_block(scope, base_depth, num_units, stride, is_bottleneck=True):
  ...
  return resnet_utils.Block(scope, bottleneck, [{
      ...
      'depth_bottleneck': base_depth,
      'stride': stride
  }]) if is_bottleneck else resnet_utils.Block(scope, resblock, [{
      'depth': base_depth,
      'stride': 1
  }] * (num_units - 1) + [{
      'depth': base_depth,
...
def resnet_v1_18(inputs,
  ...
  blocks = [
      resnet_v1_block('block1', base_depth=64, num_units=2, stride=2, is_bottleneck=False),
      resnet_v1_block('block2', base_depth=128, num_units=2, stride=2, is_bottleneck=False),
      resnet_v1_block('block3', base_depth=256, num_units=2, stride=2, is_bottleneck=False),
      resnet_v1_block('block4', base_depth=512, num_units=2, stride=1, is_bottleneck=False),
  ]
